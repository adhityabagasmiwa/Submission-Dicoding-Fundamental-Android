/*
 * Created by Adhitya Bagas on 29/12/2020
 * Copyright (c) 2020 . All rights reserved.
 */

package com.adhityabagasmiwa.dicodingsubmissionbfaa.data.db

import android.content.Context
import android.database.sqlite.SQLiteDatabase
import android.database.sqlite.SQLiteOpenHelper
import com.adhityabagasmiwa.dicodingsubmissionbfaa.data.db.DatabaseContract.UserColumns.Companion.AVATAR
import com.adhityabagasmiwa.dicodingsubmissionbfaa.data.db.DatabaseContract.UserColumns.Companion.TABLE_NAME
import com.adhityabagasmiwa.dicodingsubmissionbfaa.data.db.DatabaseContract.UserColumns.Companion.TYPE
import com.adhityabagasmiwa.dicodingsubmissionbfaa.data.db.DatabaseContract.UserColumns.Companion.USERNAME
import com.adhityabagasmiwa.dicodingsubmissionbfaa.data.db.DatabaseContract.UserColumns.Companion._ID

internal class DatabaseHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {

    companion object {

        private const val DATABASE_NAME = "db_usergithub"
        private const val DATABASE_VERSION = 1

        private const val SQL_CREATE_TABLE_USER = "CREATE TABLE $TABLE_NAME" +
                " ($_ID INTEGER PRIMARY KEY," +
                " $USERNAME TEXT NOT NULL UNIQUE," +
                " $AVATAR TEXT NOT NULL," +
                " $TYPE TEXT NOT NULL)"
    }

    override fun onCreate(db: SQLiteDatabase) {
        db.execSQL(SQL_CREATE_TABLE_USER)
    }

    override fun onUpgrade(db: SQLiteDatabase, oldVersion: Int, newVersion: Int) {
        db.execSQL("DROP TABLE IF EXISTS $TABLE_NAME")
        onCreate(db)
    }

}